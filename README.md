...
The server is a pure transformer — current UI state + an action → next state + a fresh view tree (a JSON tree of typed nodes). A thin TypeScript adapter renders it to the DOM with zero app-specific code; the browser never owns application state. Because the whole interface is data, three things come for free:
