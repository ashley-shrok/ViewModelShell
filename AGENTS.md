...
- **Always-visible inline `<input type="text">`** — the user types a plain contains value. Typing writes a draft `{operator:"contains", value:"..."}` descriptor to the bind path via `sa.write` on every keystroke (draft preservation across re-renders, same shape as any other bound input). **Pressing Enter** additionally dispatches `{name: "filter-<colKey>"}` so the server sees the applied filter and re-renders. Column-key drives the action name (`filter-title` for the "title" column, `filter-status` for "status", etc.) — mirrors the parity fixture `filter-*` catch-all convention that both backends' reset-page arms handle.
...
- **Escalation popover** — mounted in a portal div (sibling of the table wrapper, escaping `overflow-x` clip). Contains: operator picker (closed enum for the column's kind), typed value input (text input / number / date / fixed-set select / yes-no select), "Add rule" button, all-of/any-of joiner toggle, Apply / Clear / Remove-rule affordances. **Apply** commits the draft descriptor to state AND dispatches `{name: "filter-<colKey>"}`. **Clear** writes null to state AND dispatches (user expects Clear to visibly reset the table). **Outside-click / Escape** discards the draft without committing — no state write, no dispatch, filter state unchanged.
...
1. **Filter commits dispatch `{name: "filter-<colKey>"}`; per-keystroke input is draft-only (state-write, no dispatch).** The adapter writes the `FilterDescriptor` to state on every keystroke (so the value round-trips like any other bound input and is preserved across re-renders), but the actual COMMIT — inline Enter, popover Apply, popover Clear — additionally dispatches a named action so the server sees the change and re-renders. Discard paths (outside-click, Escape) neither write state nor dispatch. Without the dispatch on commit a paginated consumer's typing would have zero visible effect until an unrelated action triggered a round trip; this shape was ratified in Plan 33-06 after Ashley's real-browser exercise caught the "state-write only" Wave-1 shape as visibly broken.
