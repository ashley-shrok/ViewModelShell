...
**Selection carried across a filter change is STALE — reconcile it, don't accumulate it.** Mode C's caveat calls out the cross-*page* cost (paginating wipes the local selection set); the subtler sibling is the cross-*filter* one. If an app holds selection as its own state (e.g. a `SelectedMap`/`selectedIds` bound by per-row `CheckboxNode`s) and the operator selects rows under filter A then switches to filter B, the selection persists for rows that are **no longer visible** — and the next bulk action operates on rows the operator can't see (silently mutating them if the predicate matches, or confusingly refusing if it doesn't). This is an **app-level** concern by construction and the framework cannot fix it for you: VMS treats your state record as an opaque `TState` (it never introspects your fields), and the filtered-out rows aren't in `TableNode.rows`, so their checkbox binds aren't in the tree either — there is nothing for the framework to reach. Two correct shapes, in preference order: **(1)** where the bulk action means "act on everything matching the current filter," use the **"Select all N matching"** shape above — dispatch against the filter server-side, carry no row-id set, and there is no stale selection to go wrong. **(2)** where you genuinely need arbitrary hand-picked rows, **reconcile the selection against the visible set on every render** — prune it to `selection ∩ currently-visible-row-ids` in the same pure `BuildVm`/`buildVm` pass that produces the rows (it's derived-from-state, so it's always correct after any action), rather than carrying it as an independent accumulator that drifts from what's shown. Same principle as *Non-obvious framework behaviors*: a view/state piece recomputed every render stays honest; an accumulator fed by one action does not. (Bug class surfaced by PBMInvoices — "SelectedMap invisible carryover", found by Kara 2026-07-14; the per-render scrub is shape (2).)
