...
- **`ChatComposerNode` composite (CHAT-01..20)** — Route B composite for the chat-app compose bar: unified pill surface with growable-center textarea + fixed 34px circular leading (attach `+`/paperclip) / trailing (send / stop) icon buttons. Framework owns the intrinsic growable-center-fixed-ends layout logic that primitives cannot reach even with a Route A `FieldNode(inputType:"file", variant:"icon-only")` addition (Ashley taste-locked Panel 3 at the 2026-08-02 3-panel tasting: `banging`; Panel 2 with the icon-only file variant added still failed the visual bar because `Section(row)+heterogeneous-siblings` cannot produce a unified pill with growable-center-fixed-ends layout — the send button wraps under the attach button + textarea). Design of record at `.planning/design/composite-nodes-layer.md §4e`. **18 wire fields**: `bind` (required, draft-text state path), `sendAction` (required, ActionEvent), `placeholder`, `attachAction` (renders leading attach button when set), `attachBind` (multipart form-field name; default `"attachments"`), `maxFiles`, `maxFileSize`, `accept` (MIME types), `dropScope` (`composer|global`; closed enum), `status` (`idle|sending|streaming`; closed enum), `stopAction` (REQUIRED when status can reach streaming — both sides fail-loud; see AGENTS.md gotcha #11), `submitMode` (`enter|ctrl-enter`; closed enum; wire is kebab per KebabEnum convention), `maxRows` (auto-resize cap; default 6), `disabled`, `headerSlot`, `inputSlot`, `leadingSlot`, `trailingSlot`, `footerSlot`. **Send-button state machine** (AI-elements shape) — one button; `status` axis drives icon + click routing; non-AI consumers never set past `sending` and pay ZERO cost (they get send-with-spinner). **IME `isComposing` guard baked-in NON-OPTIONAL** (CHAT-12 CJK correctness; adversarial jsdom test is the regression gate — Enter during composition never fires send). **Three converged attach ingress paths**: click-to-picker on the attach button, drag-drop on the composer (with `dropScope` closed-enum axis; Stream Chat `WithDragAndDropUpload` precedent guarded by `dataTransfer.types.includes("Files")`), paste-image via `clipboardData.items`. Attachment preview chip strip in `headerSlot` position (framework-owned; consumer's own `headerSlot` content composes with it — both render); per-item X-remove; blob-URL image thumbs + MIME-typed file icons; `URL.revokeObjectURL` cleanup on remove + on successful send. **Backspace-on-empty removes last attachment** (AI-elements precedent; ~5 adapter lines). Attachments ride shipped multipart wire on `sendAction` dispatch under `attachBind` — a net simplification vs client-only SDKs (Stream Chat, AI Elements) that must invent presigned URLs + chunked uploads + per-attachment progress. **v1 explicitly defers** (each a separate primitive/conversation, not a v1 gap): emoji picker, @mention / /command autocomplete, voice recording, dictation, screenshot capture, model selector, typing indicator, reply-preview / edit-mode indicator as first-class fields (consumer composes in `headerSlot`).
  - **Package moves at release closeout:** npm `@ashley-shrok/viewmodel-shell` MINOR bump (target: 9.1.0 — additive wire, no breaking change); NuGet `AshleyShrok.ViewModelShell` MINOR bump byte-parallel (wire types grow in both backends together per the parity gate). Companion `AshleyShrok.ViewModelShell.Markdown` NOT affected (no rebuild required — this is a MINOR core bump, not MAJOR, so the "on any VMS core MAJOR bump" companion-rebuild rule in AGENTS.md §Conventions does NOT fire).
  - **Consumers**: **no action required** (additive optional node type). Consumers who want the composer add `<ChatComposerNode>` to their tree; existing apps see zero wire or behavior change. MIGRATION.md addition for v9.1.0: **N/A — additive.**

- **`square` icon glyph** — new shipped glyph in `viewmodel-shell/src/icons-payload.ts` (byte-parallel copy in the .NET twin). Used by `ChatComposerNode`'s send button in `status:"streaming"` state (drives the stop-icon affordance). Available to any node/composite that renders an `IconName` on rebuild.

### Changed

- **`ActionEvent.files` type widened** from `Record<string, File>` to `Record<string, File | File[]>` — supports multi-file attachments landing in the same field name (`attachBind`) via `getAll(name)` on the server (`Request.Form.Files.GetFiles(attachBind)` on .NET; `payload.files[attachBind]` on TS were already multi-file capable — only the client-adapter internal type widened). **BACKWARD-COMPATIBLE**: single-file callers see zero change (still receive a bare `File`); the union is additive on the client-consumer side.
...
- **`RichTextFieldNode` wire type (both backends)** — leaf-input primitive with `name`, `bind`, and 6 optional fields (`label`, `placeholder`, `toolbar`, `required`, `disabled`, `state`). Wire value is a **markdown string** on the bind path (via `turndown` at write time; via existing `marked` at initial-content pre-load). Consumes existing state round-trip infra (`_state` form field); zero new plumbing. Feature-surface floor (D-08): bold, italic, link, ordered/unordered lists, headings h1–h3, inline code, code block, blockquote — the Slack/GitHub level. On the .NET twin every optional field carries `[property: JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]` (nullables) or `[property: JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingDefault)]` (optional bools) per gotcha #8, so absent = absent on the wire.
- **`RichTextToolbarNode` Route B composite (both backends)** — typed slot (`tools: RichTextTool[]`) + closed-enum variance axes (`size`, `tone`, `state`). Composite shape approved via before/after tasting served on the tailnet at `http://100.113.23.63:3021/`; Ashley signed off 2026-07-31 ("`taste ok — with: fix code-block + quote editor-host rendering`", folded into Plan 28-05's CSS scope). Framework owns TipTap chain wiring, button styling, keyboard shortcuts, focus management, and a11y (aria-labels + shortcut hints); the app declares which tools appear. **When `RichTextFieldNode.toolbar` is omitted, the framework renders the DEFAULT toolbar (the full 11-tool D-08 floor) automatically** — an explicit `RichTextToolbarNode` is only needed to customize.
- **TipTap 2.x + turndown bundled into main `@ashley-shrok/viewmodel-shell` package** — lazy-imported from `browser.ts` via dynamic `import()`, following the Chart.js precedent. **Consumers who never render a `RichTextFieldNode` ship ZERO TipTap/turndown bytes** — verified by adapter test asserting the modules are not in the initial bundle graph and Vite's chunk split (`index-*.js` = 273KB, produced only on first rich-text render). No opt-in subpath, no companion package, no consumer install step required.
- **Fifth composite adopting the Phase 27 `state?: string` axis uniformity rule.** `RichTextToolbarNode` carries the freeform lifecycle axis per Phase 27's precedent — the axis is now a matter of course for row/list-shaped composites when they ship.

### Security

- **Fixed: markdown link/autolink href schemes are now sanitized at every emission site (both backends).** Prior to v8.2.0, the display-side markdown → InlineRuns pipeline (`viewmodel-shell/src/markdown.ts` + `AshleyShrok.ViewModelShell.Markdown`) emitted `InlineRun.href` values unfiltered by default — the opt-in `linkHrefRewrite` hook was the only sanitization surface, and any consumer that had not installed it was one config mistake away from a stored-XSS surface. Adversarial input `[click](javascript:alert(1))` in a markdown-string bind emitted a clickable `href="javascript:alert(1)"` on the wire; the same held for `<javascript:...>` autolinks, `data:text/html,...`, `vbscript:`, and `file:` schemes. v8.2.0 ships a shipped-by-default **WHITELIST** sanitizer at every href emission site on both backends: allowed = `http`, `https`, `mailto`, `tel`, `ftp` + relative URLs (no scheme). Everything else — including case-mixed (`JAVASCRIPT:`), whitespace-padded (`  javascript:`), or unknown-to-the-framework schemes — produces an empty href, which the plain-collapse path then absents entirely from the emitted `InlineRun` per gotcha #8. **The sanitizer runs BEFORE the opt-in `linkHrefRewrite` hook**, so a consumer hook that naively echoes its input can never re-introduce a dangerous scheme accidentally. Consumers who need to allow an additional scheme (e.g. an app-internal `myapp:` protocol) currently CANNOT — the whitelist is hard-coded. If that requirement materializes, the shape to add is a `MarkdownOptions.additionalAllowedSchemes: string[]`. Consumers who want the OPPOSITE (an even stricter whitelist) can implement it in their existing `linkHrefRewrite` hook — that hook receives the already-sanitized href, so e.g. `linkHrefRewrite: h => h.startsWith("ftp:") ? "" : h` further rejects `ftp:`. 19 byte-parallel adversarial tests ship on each backend (regular links + autolinks × 4 dangerous schemes × case-mixing + whitespace bypasses); a byte-parity fix to the .NET plain-collapse aligned the two backends' output for a sanitized link (`TextNode.value` with no `runs[]`, matching TS). See Plan 28-06 SUMMARY for the full audit table (before/after × 2 backends × 11 inputs).

### Note

- **Wire protocol token stays `viewmodel-shell/1.0`** — all wire additions are additive optional fields; no wire break; no consumer required to change code.
- **MINOR bump** — companion NuGet (`AshleyShrok.ViewModelShell.Markdown`) compile-verified against 8.2.0 headers; no republish required (MINOR core bump, not MAJOR, so AGENTS.md's "major bump = companion rebuild storm" rule does not fire). The Markdown companion IS the display-side sanitizer target described above — the Security bullet's ship point is the shipped .NET Markdown source, which downstream consumers of `AshleyShrok.ViewModelShell.Markdown` inherit on rebuild against the 8.2.0 headers (source-project-reference model per AGENTS.md).
- **Consumers who don't render a `RichTextFieldNode`** see zero effect: zero new bytes shipped (lazy-import guarantee), zero wire change, zero behavior change. The rich text path is entirely opt-in per node adoption.
- **Fail-loud on TipTap load failure** — if TipTap or turndown fails to load at runtime (e.g. offline / network-restricted environment), the framework surfaces a hard `Error` via `console.error("[ViewModelShell]", …)`. No silent no-op, no automatic fallback to a plain textarea — same fail-loud contract as the Chart.js path. This is per the capability-seam rule in AGENTS.md: capabilities with no safe core default MUST fail loudly.
...
- **`EmptyStateNode` field rename + new icon slot (COMP-08).** Old shape `{ heading, message?, action? }` → new shape `{ icon?, title, description?, action? }`. Consumers must rename `heading→title` and `message→description` in their `buildVm` code; existing `action` slot unchanged. `title` is now REQUIRED where `heading` was optional. New optional `icon?: IconName` renders in a tinted-accent-12% 3rem × 3rem circle backdrop above the title. .NET `EmptyStateNode` record additionally REMOVES the legacy `Tooltip` field (never in the TS twin — dropped for byte-alignment). This is the primary v8.0 wire break; every other v8.0 addition is additive. See `MIGRATION.md`.
...
- **Not the removed 0.15.0 `selection.action` seam.** That was the framework's *own* per-toggle seam racing with no coalescing loop. This reuses `CheckboxNode.action` — a shipped, app-declared capability — plus the non-blocking latest-wins loop (added post-0.15.0), and **never forces a dispatch semantic**: the replayed dispatch inherits the checkbox's own `blocking` choice. The app owns what a burst means, exactly as it already must for rapid manual clicking. For the non-blocking selection-tracking pattern the coalescing loop collapses the burst to **at most two round-trips** — the initial dispatch plus one coalesced latest-wins refire — **not one per row**: calls after the first overwrite a single pending-refire slot rather than queueing, so a 50-row select-all is 2 idempotent round-trips (both carry the complete post-write state), not 50. (A blocking checkbox instead serializes under the dispatch guard, as rapid manual clicking would.)
