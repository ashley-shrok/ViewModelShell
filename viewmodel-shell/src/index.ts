...
export interface SectionNode {
  type: "section";
  heading?: string;
  /** Section surface variant — the structural KIND of the section's surface (the single meaning of "variant" framework-wide). Omitted = current behavior (no modifier class). "card" emits .vms-section--card (grouped surface: background/border/padding/radius). "prose" emits .vms-section--prose (block-flow prose-typography scope for markdown/rich-text content — the Tailwind Typography "prose" plugin ported into VMS tokens: block layout with collapsing margins, asymmetric heading spacing, `h2 + *` first-child resets, real `<ul>`/`<ol>` bullets with hanging indent, prose blockquote/code-block/figure treatment). Wrap markdown-converter output in a prose section: `{ type: "section", variant: "prose", children: markdownToViewNodes(md) }`. Consumers doing app UI (dashboards, forms, tables) do NOT use prose — the default flex+gap page layout is correct for those. Closed union (D-03). */
  variant?: "card" | "prose";
