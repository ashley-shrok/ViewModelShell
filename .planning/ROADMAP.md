...
- [x] 27-08-PLAN.md — Tailnet verification page (demo/StateAxisVerification-bun/) + Ashley visual sign-off + before/after screenshots (wave 6, depends on 27-04 + 27-05)
...
**Requirements:** RICH-01, RICH-02, RICH-03, RICH-04, RICH-05, RICH-06, RICH-07, RICH-08, RICH-09
...
### Phase 29: Version-skew hard-lock — global server guard + client hard-lock modal (kill silent stale-bundle window)
...
**Success Criteria** (what must be TRUE — refined during plan-phase):
  ...
  4. Once the modal fires, all further dispatches are dropped and no re-renders paint (hard-lock is a shell-level state, not just a DOM overlay).
  5. Opt-out seam exists for consumers with pre-existing custom `onError` affordances (exact shape TBD — likely `ShellOptions.onVersionSkew: "default" | "custom"` or similar). Setting to "custom" restores current v3.8.0 behavior (fire signals via `onError`, no shipped modal); default is "default" (new hard-lock behavior).
  6. Existing per-controller `Parse(HttpRequest, currentBuild)` overload continues to compile + work (defense-in-depth layer under the global filter). No consumer code MUST change to keep working; new global default is opt-out via #5.
  7. Wire round-trips byte-identically across TS + .NET backends; both tree-validators unaffected (no new node types); `bun run parity/run.ts` green with a FeatureProbe fixture exercising stale-header 400 responses on GET.
  8. Comprehensive tailnet verification page served on 100. — 3 real scenarios: (a) initial load on stale bundle → modal, no stale render, (b) mid-session server roll-forward → next request modal, in-flight typing lost, (c) opt-out consumer with custom affordance still gets the old signals. Ashley visual sign-off recorded.
  9. Full green-tree gate green per AGENTS.md (framework vitest + core-globals + demo-types + AA-contrast + parity + all .NET Tests + Markdown companion compile).
...
- [x] 29-11-PLAN.md — Docs staging: CHANGELOG.md v9.0.0 BREAKING + MIGRATION.md v9.0.0 upgrade section + AGENTS.md new gotcha + gotcha #9 addendum (wave 9, depends on 29-10; date placeholder preserved for 29-12)
- [x] 29-12-PLAN.md — Operator-gated release: inline auth precheck + version bump (core + Markdown companion per core-major-bump rule) + npm publish + NuGet publish (core + companion) + tag v9.0.0 + advance main + announce #vms-announcements + DM Molly + PID cleanup (wave 10, depends on 29-11; autonomous: false)

### Phase 30: Chat Composer Primitive — ChatComposerNode Route B composite (v9.x, additive)

**Goal:** Ship `ChatComposerNode` — the chat-app compose bar (growable textarea + attach + send + stop-generating) as a first-class Route B composite. Angel's `/ai` compose bar (40% textarea / 40% chunky native file input / 10% send button) surfaced the gap; 3-panel tasting on tailnet proved primitives+Route-A additions can't reach the visual bar because the chat-composer shape has intrinsic layout (growable center + fixed leading/trailing + unified pill surface) that `Section(layout:"row")` with heterogeneous siblings genuinely cannot produce. Ashley taste-locked Panel 3 (composite) on 2026-08-02. Design informed by 825-line landscape survey (20+ frameworks) confirming: universal converged shape; AI-Elements state-machine send button (`status: idle|sending|streaming` closed enum, icon-swap, stopAction) is the dominant AI-chat shape and costs zero for non-AI consumers; IME `isComposing` guard is a NON-OPTIONAL correctness requirement (CJK); multi-file attach with drag-drop-on-composer + paste-image handler + above-textarea chip strip is the universal converged pattern; plain-text default (Ashley's position validated: persistent-vs-ephemeral axis, not modern-vs-legacy — every AI chat surface ships plain); typed-slots vocabulary (`leading`/`headerSlot`/`inputSlot`/`trailingSlot`/`footerSlot`) matches Ant Design X Sender's shape (VMS's closest peer for shape-fit). **v1 EXPLICITLY DEFERS:** emoji picker, @mention autocomplete, /command autocomplete, voice recording, dictation, screenshot capture, model selector, typing indicator — each a separate primitive/conversation; consumers compose via slots for v1. Additive wire (no breaking change); semver bump likely MINOR (9.1.0) given no wire break. Design of record: `~/.claude/identities/vicky/bounties/chat-composer-primitive/tasting/framework-survey.md` + `~/.claude/identities/vicky/bounties/chat-composer-primitive/tasting/` tasting panels served 2026-08-02.

**Requirements:** CHAT-01, CHAT-02, CHAT-03, CHAT-04, CHAT-05, CHAT-06, CHAT-07, CHAT-08, CHAT-09, CHAT-10, CHAT-11, CHAT-12, CHAT-13, CHAT-14, CHAT-15, CHAT-16, CHAT-17, CHAT-18, CHAT-19, CHAT-20
...
**Success Criteria** (what must be TRUE — refined during plan-phase):
  ...
  4. Attachment preview chip strip renders in `headerSlot` (framework-owned; consumer's own `headerSlot` content composes with it if both). Per-item X-remove; blob-URL image thumbs + MIME-typed file icons; `URL.revokeObjectURL` cleanup on remove/unmount.
  5. Send-button state machine (`status: "idle" | "sending" | "streaming"`) drives icon swap + click behavior. `"idle"` → send-icon fires `sendAction`; `"sending"` → spinner disabled; `"streaming"` → stop-icon fires `stopAction`. Auto-disable when `!hasSendableData` (derived from bind text + attachments count).
  6. Keyboard behavior correct across all locales: Enter=send / Shift+Enter=newline default; `submitMode:"ctrl-enter"` inverts; IME `isComposing` guard prevents CJK-composition-Enter from firing send (adversarial jsdom test proves it); Backspace-on-empty removes last attachment (~5 adapter lines).
  7. Parity fixture `chat-composer` green with `expectBodyContains` tripwires per branch (idle send / streaming stop / attach clicked / dropScope global / submitMode ctrl-enter) — no `HELPDESK_SEED=0`-style branch-hiding; every documented branch has a distinctive substring assertion per class-3 lesson.
  8. Adapter tests (jsdom) cover every interaction from CHAT-16: send-on-Enter, Shift+Enter newline, Ctrl+Enter (submitMode variant), IME composition guard, Backspace-on-empty, drag-drop landing, paste-image landing, status-transition icon swaps, stopAction dispatch on streaming click.
  9. AA-contrast hand-check green for every new fg/bg pair (send accent bg × icon; stop-icon during streaming; disabled state; chip X-remove).
  10. Framework-owned tailnet verification page served on `100.` — real bundle + real DOM + interactive; covers every wire-field combo + every slot filled/empty combo + all 12 themes. Ashley visual sign-off recorded.
  11. Angel `/ai` adopter — Angel replaces his hand-rolled compose bar with `ChatComposerNode`; visual lands cleanly; no wire regressions; Angel signs off.
  12. Full green-tree gate green per AGENTS.md (framework vitest + core-globals + demo-types + check:test-types + AA-contrast + parity + all `.NET` Tests + Markdown companion compile + `check:no-demo-style`).
...
Plans:
- [ ] 33-01-PLAN.md — Adapter build: filter-slash glyph + always-visible input + filter-icon button + icon-state grammar + escalation popover + portal positioning + keyboard + inline summary + CSS (wave 1)
- [ ] 33-02-PLAN.md — Old wire removal (8 fields from index.ts + ViewModels.cs) + TUI comment-out + tui-filter-refresh bounty + TS demo migration (Showcase, HelpDesk-bun, FeatureProbe-bun) (wave 2)
- [ ] 33-03-PLAN.md — .NET demo migration: HelpDesk AgentController + tests, ExpenseTracker tests, FeatureProbe controller (wave 2, parallel with 33-02)
- [ ] 33-04-PLAN.md — In-process adapter tests (filter-adapter.test.ts) — all 8 D-06 scenario groups + mutation-verify session (wave 3)
- [ ] 33-05-PLAN.md — Docs: MIGRATION.md v10.0.0 section + AGENTS.md typed-filter section + CHANGELOG Unreleased promotion (wave 3, parallel with 33-04)
- [ ] 33-06-PLAN.md — Full green-tree gate + served verification page + Ashley's real-browser exercise checkpoint (wave 4, autonomous: false)
