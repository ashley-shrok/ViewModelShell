...
- [post-0.4.1] bun-demo local-source consumption: `*-bun` demos must consume the framework from local source like the .NET (ProjectReference) and frontend (Vite alias) demos do — not the npm registry. Mechanism chosen = bun `link:` (live symlink) over `file:` (rejected: bun installs the file-dep's *devDependencies* → ~94-pkg vitest/jsdom bloat + a copy, not live) and over a root workspace (rejected: too large a structural change for a debt cleanup; repo is deliberately not a workspace monorepo). Cost = one `cd viewmodel-shell && bun link` step in `parity.yml` before the frozen installs (the framework dist/ already exists from the earlier AGNOSTIC-03 build step). Verified wire-neutral: full parity 7/7 byte-identical pre- vs post-fix.
...
**Last session:** 2026-05-18T13:10:00.000Z
**Stopped at:** 0.4.0 milestone CLOSED & SHIPPED (npm + NuGet 0.4.1). Post-ship maintenance this session: installed bun 1.3.14 + .NET 9.0.314 so the full parity gate runs locally; verified the docs' claimed-green baseline actually holds (parity 7/7 all backends + 176 .NET tests + 31 jsdom + 5 static guards — all green, docs accurate); then fixed the bun-demo stale-pin debt (`link:` local-source consumption + RetroBoard workaround deletion + `parity.yml` `bun link` step) with post-fix parity re-verified 7/7 byte-identical. committed on branch `fix/bun-demo-local-source` and pushed (debt fix + FeatureProbe-bun tsconfig fix + planning-doc reconciliation).
**Next action:** None pending — awaiting direction (e.g. open a PR for `fix/bun-demo-local-source`, or next milestone scope). Explicit deferrals tracked: HelpDesk requester realistic redesign; `.vms-list-item` fixed-horizontal-row finding; LAYOUT-F1.
