...
```
Pairs with `DetailListNode { type: "detail-list"; children: DetailRowNode[] }` — renders as a single `<dl>` with a fixed label column (all labels align, values start at the same X) and per-row dividers. Correct screen-reader semantics for term/definition pairs; impossible with row-wise `Section(arrange:"space-between")`.
