...
- [x] **CHAT-18**: AA-contrast hand-check for every new fg/bg pair: send button accent bg × icon fg (light + all 12 themes); stop-icon during streaming (accent kept, tone shift TBD); disabled state icon × surface bg; per-item chip preview (X-remove icon × chip bg).
