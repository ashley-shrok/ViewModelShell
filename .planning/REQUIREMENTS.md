...
- [x] **NBA-01**: A dispatch can carry `blocking: false` (optional; default `true` → existing apps byte-unchanged). A non-blocking (silent) round-trip no longer occupies the single global dispatch mutex: a user action fired while a non-blocking round-trip is in flight is honored, not silently dropped, and vice versa.
