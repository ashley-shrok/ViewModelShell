...
- [ ] **EXAMPLES-01**: Showcase renders a canonical reference set (at least: dashboard, form-heavy page, list/detail) using only `.vms-*` nodes + the shipped stylesheet, visually serviceable benchmarked against Bootstrap's example pages.
- [ ] **EXAMPLES-02**: Every demo app imports the shipped stylesheet and contains no hand-rolled per-demo `<style>` page chrome.
