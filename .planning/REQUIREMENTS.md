...
- [ ] **NBA-01**: A dispatch can carry `blocking: false` (optional; default `true` → existing apps byte-unchanged). A non-blocking (silent) round-trip no longer occupies the single global dispatch mutex: a user action fired while a non-blocking round-trip is in flight is honored, not silently dropped, and vice versa. — _GAP (14-VERIFICATION.md): partially met. `blocking:false` field + independent lanes exist, but two confirmed/reproduced defects mean lanes can still silently drop/misroute each other's outcome — CR-01 (coalesced refire loses its own classification → a poll can be misrouted into the blocking lane and halt the poll loop) and CR-02 (global non-lane-aware epoch → a slow blocking response is silently discarded when a later non-blocking one resolves first). Close via `/gsd:plan-phase 14 --gaps`._
...
- [x] **NBA-04**: `blocking` is absent-when-default on BOTH backends (F2 `WhenWritingDefault` / TS optional); the wire token stays `viewmodel-shell/1.0`; `bun run parity/run.ts` is byte-identical green with new fixtures for non-blocking dispatch, coalesced rapid fire, and out-of-order discard.
