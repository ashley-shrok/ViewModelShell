...
- **Canonical examples / few-shot surface** — Showcase + demos benchmarked against Bootstrap example pages (Milestone 0.4.0, Phase 5)
- **0.4.0 aligned version bump + release closeout** — npm `0.3.14`→`0.4.0`, NuGet `0.3.10`→`0.4.0`, MIGRATION/CHANGELOG (Milestone 0.4.0, Phase 5 RELEASE-01/03 — the layout enum is the wire-format change that forces the aligned minor)
...
---
*Last updated: 2026-05-18 — Phase 4 (Preset-Grid Layout) complete: LAYOUT-01..05 validated — `layout?: "stack" | "split" | "cards"` closed-union added to existing `PageNode`/`SectionNode` (both backends; `src/server.ts` via re-export; single shared `ViewModels.cs`), renderer emits `.vms-{page,section}--{split,cards}` only for split/cards (omitted/`stack` DOM byte-identical), two pure-CSS intrinsic presets with one additive `--vms-card-min: 16rem` and zero `@media`/`container-type`, FeatureProbe parity fixture widened (layout+density+card) byte-identical .NET/Bun/Node with the 7 existing fixtures still green (closed the Phase 3 D-05 deferral). vitest 31/31, code review clean, no version bump (D-11). Next: Phase 5 Canonical Examples + 0.4.0 release closeout (demos/Showcase on shipped stylesheet, Bootstrap-benchmarked set, aligned npm/NuGet 0.4.0 bump + MIGRATION/CHANGELOG).*
