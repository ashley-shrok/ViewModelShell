...
// — no backend, just enough local state to make the modal interactive.
  ...
  return {
    ...
    children: [
      ...
      { type: "section", heading: "Modal", children: [
        { type: "text", value: "Click below to reopen the modal. Cancel/Delete forever/the X all dismiss it.", style: "muted" },
        { type: "button", label: "Open modal", action: { name: "modal:open" }, variant: "primary" },
      ]},

      // Modal lives at page level so the backdrop covers the whole viewport.
      ...(modalShown ? [{
        type: "modal" as const,
...
function handle(action: ActionEvent): void {
  ...
  if (action.name === "modal:dismiss") modalShown = false;
  if (action.name === "modal:open")    modalShown = true;
